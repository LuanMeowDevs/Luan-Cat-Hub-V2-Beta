...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto farm Observation Haki (Ken) - Sea 1, 2 and 3",
...
local _lastInputPos = nil;
local _freeMoveCancelled = false;
game:GetService("UserInputService").InputBegan:Connect(function(inp)
	if shouldTween then
		if inp.UserInputType == Enum.UserInputType.Touch then
			_lastInputPos = inp.Position;
		end;
		if inp.KeyCode == Enum.KeyCode.W or inp.KeyCode == Enum.KeyCode.A
			or inp.KeyCode == Enum.KeyCode.S or inp.KeyCode == Enum.KeyCode.D then
			_freeMoveCancelled = true;
			shouldTween = false;
			_G.StopTween = true;
		end;
	end;
end);
game:GetService("UserInputService").TouchMoved:Connect(function(inp)
	if shouldTween and _lastInputPos then
		local delta = (inp.Position - _lastInputPos).Magnitude;
		if delta > 8 then
			_freeMoveCancelled = true;
			shouldTween = false;
			_G.StopTween = true;
...
MainTab:AddToggle({
	Title = "Accept Quests",
	Desc = "Automatically accepts quest before farming",
	Value = false,
	Callback = function(v)
...
getgenv().FastM1FruitEnabled = false;
SettingsTab:AddToggle({
	Title = "Fast M1 Fruit",
	Desc = "Rapidly spams fruit M1 when Weapon is set to Fruit and any farm is active.",
	Value = false,
	Callback = function(state)
		getgenv().FastM1FruitEnabled = state;
		(getgenv()).SaveSetting();
	end
});
spawn(function()
	...
	while true do
		task.wait(0);
		if getgenv().FastM1FruitEnabled and (_G.ChooseWP == "Fruit" or _G.Settings.Main["Select Weapon"] == "Fruit") and (
			_G.EclipseStartFarm or _G.Settings.Main["Auto Farm"] or
			_G.Settings.Main["Auto Farm Fruit Mastery"] or
			_G.Settings.SeaEvent["Sail Boat"] or _G.SailBoats or
			_G.Settings.Main["Auto Farm All Boss"] or _G.Settings.Main["Auto Farm Boss"]
		) then
			pcall(function()
				local char = game.Players.LocalPlayer.Character;
				if not char then return; end;
				local hum = char:FindFirstChildOfClass("Humanoid");
				if not hum or hum.Health <= 0 then return; end;
				vim2:SendMouseButtonEvent(512, 300, 0, true, game, 0);
				task.wait(0.04);
				vim2:SendMouseButtonEvent(512, 300, 0, false, game, 0);
				task.wait(0.04);
			end);
		else
			task.wait(0.05);
...
SailViaTikiToggle = SeaEventTab:AddToggle({
	...
	Desc = "Goes to Tiki Outpost, buys the selected boat, mounts and sails to the chosen sea.",
	Value = false,
	Callback = function(state)
		...
		pcall(function()
			...
			if (hrp.Position - zoneCF.Position).Magnitude > 500 then
				pcall(function()
					...
					if boatSeat and (hrp.Position - zoneCF.Position).Magnitude > 300 then
						boat:SetPrimaryPartCFrame(CFrame.new(zoneCF.Position.X, 30, zoneCF.Position.Z));
						task.wait(0.1);
						hrp.CFrame = boatSeat.CFrame * CFrame.new(0, 1.5, 0);
					end;
				end);
			end
		end)
...
AutoSailBoatToggle = SeaEventTab:AddToggle({
	...
	Desc = "Automatically buys boat, navigates and stays in the selected farm zone attacking sea events.",
...
SeaEventTab:AddSection("Sea Beast - Fruit M1 Only Detector");
SeaEventTab:AddParagraph({
	Title = "Sea Beast Fruit Detector",
	Desc = "When Fast M1 Fruit is enabled, detects if the Sea Beast has one of these fruits: Pain, T-Rex, Kitsune, Control, Dragon (East or West). If detected, uses only M1 (no skills spam)."
});
getgenv().SeaBeastFruitOnly = false;
local _seaBeastFruitList = {"Pain", "T-Rex", "T Rex", "Kitsune", "Control", "Dragon East", "Dragon West", "Dragon"};
local function _checkSeaBeastFruitType()
	for _, enemy in pairs(workspace.Enemies:GetChildren()) do
		if enemy.Name:lower():find("sea beast") or enemy.Name:lower():find("seabeast") then
			if enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
				for _, fruitName in pairs(_seaBeastFruitList) do
					if enemy:FindFirstChild(fruitName) or enemy:FindFirstChild(fruitName .. " Fruit") then
						return true, fruitName;
					end;
					for _, desc in pairs(enemy:GetDescendants()) do
						if desc.Name:lower():find(fruitName:lower()) then
							return true, fruitName;
						end;
					end;
				end;
...
spawn(function()
	while task.wait(0.3) do
		if _G.Settings.SeaEvent["Sail Boat"] or _G.SailBoats then
			pcall(function()
				local found, fruitName = _checkSeaBeastFruitType();
				getgenv().SeaBeastFruitOnly = found;
				if found and getgenv().FastM1FruitEnabled then
					Library:Notify({
						Title = "Sea Beast Detector",
						Content = "Sea Beast with " .. tostring(fruitName) .. " detected! Using M1 only.",
						Icon = "alert",
						Duration = 4
					});
				end;
...
task.spawn(function()
	while true do
		...
		pcall(function()
			...
			if pirateRaid then
				_pirateRaidStatusParagraph:SetDesc("ACTIVE in server!");
			else
...
task.spawn(function()
	while true do
		...
		pcall(function()
			...
			if factory then
				_factoryStatusParagraph:SetDesc("ACTIVE in server!");
			else
...
HoldAndSkillTab:AddParagraph({
	Title = "How It Works",
	Desc = "Select below which keys/skills will be used in ALL farms and hub functions that need skills. The combinations Z X C Melee, Z X C V F Fruit, Z X Sword, and Z X Gun define quick groups."
});


HoldAndSkillTab:AddToggle({
	Title = "Z X C Melee",
	Desc = "Uses only Z, X, C of Melee (Fighting Style)",
	Value = false,
	Callback = function(state)
		if state then
...
HoldAndSkillTab:AddToggle({
	...
	Desc = "Uses Z, X, C, V and F of Fruit (Devil Fruit)",
	Value = false,
	Callback = function(state)
		if state then
			getgenv().HoldSkillConfig["Z"] = true;
...
HoldAndSkillTab:AddToggle({
	Title = "Z X Sword",
	Desc = "Uses only Z and X of Sword",
	Value = false,
	Callback = function(state)
		if state then
			getgenv().HoldSkillConfig["Z"] = true;
